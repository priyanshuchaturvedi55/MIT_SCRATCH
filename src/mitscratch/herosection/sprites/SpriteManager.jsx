  ...
  const executeBlock = useCallback((sprite, block) => {
    return new Promise((resolve) => {
      switch (block.type) {
        ...
        case 'goto':
          const gotoX = parseInt(block.x) || 0;
          const gotoY = parseInt(block.y) || 0;
          updateSpritePosition(sprite.id, gotoX, gotoY);
          setTimeout(resolve, 500);
          break;

        case 'say':
          const sayMessage = block.placeholder || 'Hello!';
          const sayDuration = (parseInt(block.unit) || 2) * 1000;
          showSpriteMessage(sprite.id, sayMessage, sayDuration);
          setTimeout(resolve, sayDuration);
          break;

        case 'think':
          const thinkMessage = block.placeholder || 'Hmm...';
          const thinkDuration = (parseInt(block.unit) || 2) * 1000;
          showSpriteMessage(sprite.id, `💭 ${thinkMessage}`, thinkDuration);
          setTimeout(resolve, thinkDuration);
          break;

        case 'repeat':
          const repeatCount = parseInt(block.placeholder) || 1;
          // For repeat blocks, we'll handle them in the execution loop
          resolve();
          break;
